    ...
    <div className="page-wrapper">
      <div className="page-card">
        ...
        <div className="perfil-container">
          <Avatar size={72} style={{ background: 'linear-gradient(135deg,#0094A2,#5B5FE3)', fontSize: 24, fontWeight: 700 }}>
            {docente.iniciales}
          </Avatar>
          <div>
            <h2 className="perfil-nombre">{docente.nombre}</h2>
            <p className="perfil-depto">{docente.carrera}</p>
            <Tag style={{ background: 'rgba(0,148,162,0.1)', border: '1px solid rgba(0,148,162,0.3)', color: '#0094A2', borderRadius: 6 }}>
        ...
        <div className="opciones-grid">
          <button className="opcion-btn" onClick={() => ir('/sistema/uso-biblioteca')}>
            <ReadOutlined style={{ fontSize: 26, color: '#0094A2' }} />
          ...
          <button className="opcion-btn" onClick={() => ir('/sistema/prestamo')}>
            <BookOutlined style={{ fontSize: 26, color: '#5B5FE3' }} />
          ...
          {docente.prestamosActivos > 0 && (
            <button className="opcion-btn opcion-devolucion" onClick={() => ir('/sistema/devolucion')}>
              <RollbackOutlined style={{ fontSize: 26, color: '#FF6B35' }} />
              <span><span className="opcion-titulo">Devolución de libro</span><span className="opcion-desc">{docente.prestamosActivos} préstamo{docente.prestamosActivos > 1 ? 's' : ''} activo{docente.prestamosActivos > 1 ? 's' : ''}</span></span>
            </button>
          )}
        </div>
      </div>
