...
function Prestamo({ docente, onTerminar, enModal }: Props) {
  ...
  useEffect(() => {
    if (!texto && !programaSeleccionado) { setResultados([]); return }
    const t = setTimeout(() => {
  ...
  if (confirmado) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      <SwapOutlined style={{ fontSize: 56, color: '#00796B', marginBottom: 16 }} />
  ...
  const contenido = (
    <>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <SwapOutlined style={{ fontSize: 36, color: '#00796B', marginBottom: 12 }} />
      ...
      <div className="form-field">
        ...
        <Input placeholder="Escribe para buscar..." value={texto}
          onChange={e => setTexto(e.target.value)}
          prefix={<SearchOutlined style={{ color: '#94A3B8' }} />} size="large" />
      </div>

      {resultados.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20, maxHeight: 240, overflowY: 'auto' }}>
          {resultados.map(libro => (
            <div key={libro.id}
              className={`libro-opcion ${libroSeleccionado?.id === libro.id ? 'selected' : ''}`}
              onClick={() => setLibroSeleccionado(libro)} style={{ cursor: 'pointer' }}>
              <div style={{ flex: 1 }}>
        ...
        <>
          <div className="libro-resultado" style={{ marginBottom: 20 }}>
            ...
            {libroSeleccionado.disponibles === 0 && (
              <div className="no-disponible">No hay ejemplares disponibles en este momento.</div>
            )}
          </div>
            ...
            <>
              <div className="documento-row" style={{ marginBottom: 18 }}>
                ...
                <div className="form-field" style={{ marginBottom: 0 }}>
                  ...
                  <Input placeholder="N° de documento" value={numeroDocumento}
                    onChange={e => setNumeroDocumento(e.target.value)} size="large" />
                </div>
              </div>
              ...
              <div className="plazo-card">
                ...
                {!mostrarCalendario ? (
                  ...
                  <DatePicker
                    ...
                    onChange={(f) => { if (f) { setFechaDevolucion(f); setMostrarCalendario(false) } }}
                    format="DD/MM/YYYY"
                    disabledDate={(f) => f.isBefore(dayjs(), 'day')}
                    autoFocus open
                    onOpenChange={(o) => { if (!o) setMostrarCalendario(false) }}
                  />
                )}
              </div>

              <Button className="btn-confirmar" block size="large"
                onClick={handleConfirmar} loading={guardando}>
                Confirmar préstamo
              </Button>
            </>
          )}
  ...
  return (
    <div className="page-wrapper">
      <div className="page-card" style={{ maxWidth: 720 }}>{contenido}</div>
    </div>
  )
}
