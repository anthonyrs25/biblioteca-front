  ...
  if (confirmado) return (
    <div className="page-wrapper">
      <div className="page-card" style={{ textAlign: 'center' }}>
        <CheckCircleOutlined style={{ fontSize: 64, color: '#8b5cf6', marginBottom: 16 }} />
    ...
    <div className="page-wrapper">
      <div className="page-card">
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <RollbackOutlined style={{ fontSize: 40, color: '#8b5cf6', marginBottom: 12 }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
          {librosDocente.map(libro => (
            <div key={libro.codigo} className={`libro-opcion ${seleccionado === libro.codigo ? 'selected' : ''}`} onClick={() => setSeleccionado(libro.codigo)}>
              <BookOutlined style={{ color: '#8b5cf6', fontSize: 18 }} />
              <div style={{ flex: 1 }}>
                <p className="libro-titulo" style={{ margin: 0 }}>{libro.titulo}</p>
                <p className="libro-autor" style={{ margin: 0 }}>{libro.autor}</p>
              </div>
              <Tag style={{ background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)', color: '#8b5cf6', borderRadius: 6 }}>{libro.codigo}</Tag>
            </div>
          ))}
        </div>
